...
const TopRecruiters = () => {
  ...
  const marketingLogos = Array.from(
    { length: 12 },
    (_, i) => `/Logo/BBA – Marketing/logo/${i + 1}.jpg`,
  );
  const financeLogos = Array.from(
    { length: 10 },
    (_, i) => `/Logo/BBA – Finance/logo/${i + 1}.jpg`,
  );
  const ibLogos = Array.from(
    { length: 10 },
    (_, i) => `/Logo/BBA International Business/logo/${i + 1}.jpg`,
  );
  const hrLogos = Array.from(
    { length: 10 },
    (_, i) => `/Logo/BBA HR/logo/${i + 1}.jpg`,
  );
  const logisticsLogos = Array.from(
    { length: 10 },
    (_, i) => `/Logo/BBA Logistics & Supply Chain Management/logo/${i + 1}.jpg`,
  );
        ...
        <div className="relative flex overflow-x-hidden">
          <div className="animate-marquee flex whitespace-nowrap gap-6 items-center py-4">
            {row1.concat(row1).map((logo, index) => (
              <div
                key={`row1-${index}`}
                className="flex-shrink-0 bg-white rounded-xl flex items-center justify-center p-3 w-[180px] h-[75px] md:w-[220px] md:h-[95px] shadow-lg"
              >
                <img
                  src={logo}
                  alt="Recruiter Logo"
